...
void massPlot() {
	...
	TGraph* g_exp = plots.getExpMassLimitGluino();
  	...
  	TH1 * h;
  	h = canvas->DrawFrame(200., .1, 900., 5e2);
  	...
  	// details
  	TPaveText* blurb = new TPaveText(205., 1.e1, 450., 4.5e2);
  	...
  	// legend
  	TBox *legbg = new TBox(500., 1.e1, 750., 4e2);
  	legbg->Draw();
  	TLegend *leg = new TLegend(500., 1.e1, 750., 4e2,"95% C.L. Limits","");
