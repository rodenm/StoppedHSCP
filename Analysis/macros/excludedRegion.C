...
void excludedRegion() {
	...
	double data[32]  = { 70.,       641.656, 665.193, 26.2761, 38.2983, 54.5252, 71.4299, 
			     100.,	683.485, 682.54,  29.1804, 45.2625, 59.2212, 82.6958,
			     150.,	717.43,  694.084, 30.6885, 49.8235, 62.7341, 70.0266,
			     200.,	715.187, 699.153, 30.8701, 50.8663, 64.3211, 64.8578 };
	...
	TCanvas canvas("gluinoLimitVsE", "", 800, 600);
	...
	blurb->AddText("#int L dt = 4.0 fb^{-1}");//,  #int L_{eff} dt = 935 pb^{-1}");
	...
	TCanvas canvas2("gluinoLimitVsEVis", "", 800, 600);
	...
	blurb2.AddText("#int L dt = 4.0 fb^{-1}");//,  #int L_{eff} dt = 935 pb^{-1}");
  	//blurb2.AddText("L^{max}_{inst} = 3.5 #times 10^{33} cm^{-2}s^{-1}");
  	blurb2.AddText("#sqrt{s} = 7 TeV");
	...
	disallowed.SetPoint(1, 600., 600.);
	disallowed.SetPoint(2, 600., 0.);
...
//	TPaveText blurb3(0.48, 0.15, 0.6, 0.37, "NDC");
	TPaveText blurb3(0.72, 0.15, 0.92, 0.37, "NDC");
	blurb3.AddText("CMS 2011");
	blurb3.AddText("#int L dt = 4.0 fb^{-1}");//,  #int L_{eff} dt = 935 pb^{-1}");
	//blurb3.AddText("L^{max}_{inst} = 3.5 #times 10^{33} cm^{-2}s^{-1}");
	blurb3.AddText("#sqrt{s} = 7 TeV");
	...
	TLatex t1(0.44, 0.42, "E_{thresh} > 70 GeV");
	t1.SetNDC(true);
	t1.SetTextFont(42);
	t1.SetTextSize(0.03);
	t1.SetTextAngle(25);
