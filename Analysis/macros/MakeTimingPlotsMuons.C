...
TC1->SaveAs("Plots/TimingMuonOuter.pdf");


TCanvas *TC3 = new TCanvas;
TC3->cd();
TH2D * hist3 = new TH2D("hist3","hist3",20,0,1,20,0,1);
CR->GetChain(3)->Draw("TimingRightPeak:TimingFracRightNextRight>>hist3",(Topological && TCut("TimingFracRightNextRight>0")));
hist3->SetYTitle("BX_Peak+1 / BX_Peak");
hist3->SetXTitle("BX_Peak+2 / BX_Peak+1");
hist3->Draw("cont0");
TC3->SaveAs("Plots/TimingMuon2d.pdf");

TCanvas * TC4= new TCanvas;
TC4->cd();
hist3->Draw("surf1");
TC4->SaveAs("Plots/TimingMuon3d.pdf");
...
hist5->Draw();
TC5->SaveAs("Plots/TimingMuonFracLeader.pdf");
