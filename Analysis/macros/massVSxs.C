...
void massVSxs(double lumi=-1.,double maxInstLumi=-1.) 
{
  if (lumi<0)
    lumi=18.6;
  if (maxInstLumi<0)
  ...
  // graphs - observed
  TGraph* g_gluino  = plots.getLimitGluino();
  TGraph* g_stop    = plots.getMassLimitGluino();
  ...
  leg->SetTextFont(42);
  leg->SetFillColor(0);
  leg->AddEntry(g_gluino, "Lifetime limit : m_{#tilde{g}}=500 GeV/c^{2}, m_{#tilde{#chi^{0}}}=387 GeV/c^{2}", "l");
  leg->AddEntry(g_stop, "Mass limit  : m_{#tilde{t}}=300 GeV/c^{2}, m_{#tilde{#chi^{0}}}=109 GeV/c^{2}", "l");
  leg->Draw();
