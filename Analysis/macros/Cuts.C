...
bool cut(StoppedHSCPEvent* evt, unsigned cuts) {
  bool result=true;
  if (cuts & 0x1 > 0) result &= (evt->getJet(0).eta < CutJetEtaCut);
  if (cuts & 0x2 > 0) result &= (evt->getJet(0).n60 < CutJetN60Cut);
  if (cuts & 0x4 > 0) result &= (evt->getJet(0).n90 > CutJetN90Cut);
  if (cuts & 0x8 > 0) result &= (evt->getJet(0).e > CutJetEnergyCut);
  if (cuts & 0x10 > 0) result &= (evt->nMuon() < CutMuonVeto);
//  if (cuts & 0x1f0 > 0) {
//
//     // sum 5 highest digis
...
//     if (cuts & 0x20 > 0) result &= (r1 > 0.15 && r2 > 0.1);
//     if (cuts & 0x40 > 0) result &= (r2 > 0.5);
//     if (cuts & 0x80 > 0) result &= (peakfrac > 0.4 && peakfrac < 0.7);
//     if (cuts & 0x100 > 0) result &= (outfrac < 0.1);
//   }
  return result;
}
