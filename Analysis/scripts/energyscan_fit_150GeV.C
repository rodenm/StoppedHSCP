{
  ...
  Double_t countBoth[nGBothpoints] = {0.0, 0.0, 3.0, 0.0, 4.0, 12.0, 8.0, 188.0, 126.0, 354.0, 677.0, 
				      1430.0, 1560.0, 1411.0, 1539.0};
  ...
  for (int ib = 0; ib < nGBothpoints; ib++) {
    ...
    //std::cout << bothyGplus1[ib]  << "\t"<<bothyG[ib] <<"\t"<<bothyGminus1[ib] <<std::endl;
  }

  Double_t xaxisG300[nG300points] = {50.,  75.,  100., 125., 133.};
  Double_t countG300[nG300points] = {0.,   0.,   8.,   188., 354.};
  ...
  Double_t countG600[nG600points] = {0.,   3.,   4.,   12.,  126., 677., 1430.,1560.,1411.,1539.,};
  ...
  gfit->SetParameters(0.21,0.028,4.3,0.20);
  gfit->SetLineColor(kWhite);
  gfit->SetRange(100,110);
  gfit->SetLineWidth(0);
  //gfit->Draw("FC same");

  TF1* gfitp = new TF1("gfitp","[0]*TMath::Erf([1]*x-[2])+[3]",70,300);
  gfitp->SetParameters(0.21,0.034,5.14,0.207);
  ...
  gfitm->SetParameters(0.20,0.034,5.14,0.207);
  ...
  Double_t countBothS[nSBothpoints] = {2.0, 2.0, 0.0, 10.0, 16.0, 116.0, 258.0, 478.0, 429.0, 
				       663.0, 755.0, 678.0, 561.0, 689.0, 736.0, 829.0, 660.0, 
				       676.0, 624.0, 721.0, 700.0, 636.0, 704.0, 681.0};
  ...
  Double_t countS300[nS300points] = {2., 0., 16., 429.};
  ...
  Double_t countS400[nS400points] = {2., 10., 116., 258., 478., 663., 755., 678., 689., 736., 829.};
  ...
  Double_t countS500[nS500points] = {561., 660., 624., 721., 636.};
  ...
  Double_t countS600[nS600points] = {676., 700., 681., 704.};
  ...
  sfit->SetParameters(0.4,0.028,2.5,0.013);
  sfit->SetLineColor(kBlack);
  sfit->SetLineWidth(2);
  sfit->SetRange(50,320);

  TF1* sfitp = new TF1("sfitp","[0]*TMath::Erf([1]*x-[2])+[3]",0,300);
  sfitp->SetParameters(0.25,0.034,2.33,0.25);
  sfitp->SetRange(50,320);
  ...
  sfitm->SetParameters(0.25,0.034,2.33,0.25);
  ...
  canvas->Print("summer12_8TeV_detectionEff_150GeV.pdf");  
}
