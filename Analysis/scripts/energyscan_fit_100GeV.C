{
  ...
  Double_t countBoth[nGBothpoints] = {0.0, 2.0, 42.0, 47.0, 343.0, 684.0, 780.0, 1445.0, 1454.0,
				      1532.0, 1598.0, 1671.0, 1693.0, 1511.0, 1585.0};
  Double_t totalBoth[nGBothpoints] = {3578.0, 3648.0, 3640.0, 3581.0, 3577.0, 3641.0, 3655.0, 
				      3652.0, 3634.0, 3564.0, 3640.0, 3642.0, 3637.0, 3429.0, 3643.0};
  ...
  for (int ib = 0; ib < nGBothpoints; ib++) {
    ...
    bothyGplus1[ib] = bothyG[ib]>0 ? bothyG[ib] + bothErrG[ib]:0.;
    bothyGminus1[ib] = bothyG[ib]>0 ? bothyG[ib] - bothErrG[ib]:0.;
  }

  Double_t xaxisG300[nG300points] = {50.,75.,100.,125.,133.};
  Double_t countG300[nG300points] = {2., 47., 780., 1445., 1532.,};
  ...
  Double_t countG600[nG600points] = {0., 42., 343., 684., 1454., 1598., 1671., 1693., 1511., 1585.,};
  ...
  gluino->Draw("P same"); 
  gluinoPlus->Draw("psame");
  gluinoMinus->Draw("p same");
  
  TF1* gfit = new TF1("gfit","[0]*TMath::Erf([1]*x-[2])+[3]",70,300);
  gfit->SetParameters(0.2199,0.0494,5.06,0.22);
  gfit->SetLineColor(kBlack);
  gfit->Draw("FC same");

  TF1* gfitp = new TF1("gfitp","[0]*TMath::Erf([1]*x-[2])+[3]",70,300);
  ...
  TF1* gfitm = new TF1("gfitm","[0]*TMath::Erf([1]*x-[2])+[3]",70,300);
  ...
  Double_t countBothS[nSBothpoints] = {95.0, 145.0, 247.0, 526.0, 668.0, 711.0, 925.0, 949.0, 826.0,
				       1022.0, 1050.0, 861.0, 713.0, 864.0, 914.0, 988.0, 814.0, 783.0, 
				       726.0, 800.0, 792.0, 726.0, 755.0, 752.0};
  ...
  Double_t countS300[nS300points] = {95., 247., 668., 826.};
  ...
  Double_t countS400[nS400points] = {145., 526., 711., 925., 949., 1022., 1050., 861., 864., 914., 988.};
  ...
  Double_t countS500[nS500points] = {713., 814., 726., 800., 726.};
  ...
  Double_t countS600[nS600points] = {783., 792., 752., 755.};
  ...
  stop->SetTitle("stop stopping efficiency;E_{gluon|top} [GeV];#varepsilon_{det}");
  ...
  stop300->SetMarkerColor(kBlue-4);
  stop300->SetMarkerStyle(20);
  stop300->SetMarkerSize(1.15);
  stop300->SetLineWidth(2);

  stop400->SetLineColor(kBlue+1);
  stop400->SetMarkerColor(kBlue+1);
  stop400->SetMarkerStyle(22);
  stop400->SetMarkerSize(1.15);
  stop400->SetLineWidth(2);

  stop500->SetLineColor(kBlue-2);
  stop500->SetMarkerColor(kBlue-2);
  stop500->SetMarkerStyle(21);
  stop500->SetMarkerSize(1.15);
  stop500->SetLineWidth(2);

  stop600->SetLineColor(kBlue+3);
  stop600->SetMarkerColor(kBlue+3);
  stop600->SetMarkerStyle(23);
  ...
  //sfit->SetParameters(0.1,0.02,2.,0.1);
  //TF1* sfit = new TF1("sfit","[0]*TMath::Erf(.0495*x-5.06)+[1]",0,300);
  //sfit->SetParameters(0.1,0.01);
  TF1* sfit = new TF1("sfit","[0]*TMath::Erf([1]*x-5.06)+[2]",89,300);
  sfit->SetParameters(0.14,0.05,0.14);
  sfit->SetLineColor(kWhite);
  sfit->SetRange(89,320);

  //TF1* sfitp = new TF1("sfitp","[0]*TMath::Erf([1]*x-[2])+[3]",0,300);
  //sfitp->SetParameters(0.1,0.02,2.,0.1);
  //TF1* sfitp = new TF1("sfitp","[0]*TMath::Erf(.0495*x-5.06)+[1]",0,300);
  //sfitp->SetParameters(0.1,0.01);
  TF1* sfitp = new TF1("sfitp","[0]*TMath::Erf([1]*x-5.06)+[2]",89,300);
  sfitp->SetParameters(0.14,0.05,0.14);
  sfitp->SetRange(89,320);
  ...
  //sfitm->SetParameters(0.1,0.02,2.,0.1);
  //TF1* sfitm = new TF1("sfitm","[0]*TMath::Erf(.0495*x-5.06)+[1]",0,300);
  //sfitm->SetParameters(0.1,0.01);
  TF1* sfitm = new TF1("sfitm","[0]*TMath::Erf([1]*x-5.06)+[2]",89,300);
  sfitm->SetParameters(0.14,0.05,0.14);
  sfitm->SetLineColor(kBlue-10);
  sfitm->SetLineWidth(7);
  sfitm->SetFillColor(10);
  sfitm->SetFillStyle(1001);
  sfitm->SetRange(89,320);
  sfitm->Draw("FC same");
  ...
  leg->SetTextFont(42);
  leg->SetHeader("CMS Simulation,    #sqrt{s} = 7 TeV");
  ...
  canvas->Print("summer12_8TeV_detectionEff_100GeV.pdf");
}
