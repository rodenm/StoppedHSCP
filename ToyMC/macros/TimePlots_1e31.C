...

{
  ...
  TPaveText blurb(13., 9., 21., 10.8);
  //TText* t2=blurb.AddText("#sqrt{s} = 10 TeV");
  TText* t3=blurb.AddText("L_{inst} = 10^{31}cm^{-2}s^{-1}");
...
//   t6.Draw();
  ...
  TLegend leg(22., 8., 29., 10.8, "Gluino lifetime", ""); //#tau_{#tilde{g}}", "");
...
//   t7.Draw();
  ...
  TLatex roots(12.1, .35, "EXO-09-001 scaled to #sqrt{s} = 7 TeV");
  roots.SetTextSize(0.03);
  roots->Draw();


  c->Update();
  c->Print("SigVsTime_Combined_m300_1e31.png");
  c->Print("SigVsTime_Combined_m300_1e31.eps");
